...
class CANBootloaderFlash:
    ...
    def write_4bytes(self, data: bytes, max_retries: int = 3) -> bool:
        """
        ...
        Args:
            ...
            max_retries: Maximum retries on stale READY messages
        ...
        for retry in range(max_retries):
            ...
            if resp.data[0] == RESP_ACK:
                ...
                # Got heartbeat instead of ACK - clear queue and retry
                if retry < max_retries - 1:
                    self.driver.clear_receive_queue()
                    continue
                else:
                    print(f"\n✗ Write failed: Too many stale READY messages")
                    return False
            else:
                print(f"\n✗ Write failed: Unexpected response 0x{resp.data[0]:02X}")
                return False
